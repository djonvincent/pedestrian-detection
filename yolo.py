...
def drawPred(image, class_name, confidence, depth, left, top, right, bottom, colour):
    # Draw a bounding box.
    ...
    cv2.rectangle(image, (left, top - round(labelSize[1])),
        (left + round(labelSize[0]), top + baseLine), (255, 255, 255), cv2.FILLED)
    cv2.putText(image, label, (left, top), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0), 1, cv2.LINE_AA)
