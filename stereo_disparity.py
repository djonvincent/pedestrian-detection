...
master_path_to_dataset = "/home/dion/TTBB-durham-02-10-17-sub10"; # ** need to edit this **
...
skip_forward_file_pattern = "1506943060.477936"; # set to timestamp to skip forward to
...
for filename_left in left_file_list:
    ...
    if ('.png' in filename_left) and (os.path.isfile(full_path_filename_right)) :
        ...
        raw_disparity = stereoProcessor.compute(grayL,grayR);

        # filter out noise and speckles (adjust parameters as needed)

        dispNoiseFilter = 5; # increase for more agressive filtering
        cv2.filterSpeckles(raw_disparity, 0, 4000, max_disparity - dispNoiseFilter);
        ...
        _, disparity = cv2.threshold(raw_disparity,0, max_disparity * 16, cv2.THRESH_TOZERO);
        ...
        for i in range(0, len(boxes)):
            ...
            roi = raw_disparity[max(top,0):top+height, max(left,0):left+width]
            depth = 1000/(np.median(roi[roi>0]))
            if depth == np.inf:
                print(np.amin(roi))
                print(np.median(roi))
            if depth == -1:
                continue
            #depth = 100/disparity_scaled[centre[1], centre[0]]
            #cv2.circle(imgL, centre, 10, (255, 178, 50), 10)
            yolo.drawPred(imgL, yolo.classes[classIDs[i]], confidences[i], depth, left, top, left + width, top + height, (255, 178, 50))
